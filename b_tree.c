...
Row* search(Page *page, int wanted_element) {
	uint32_t i = 0;
	while (i <= page->amount_of_keys && (page->info[i] != NULL && wanted_element > page->info[i]->id)) {
		i++;	
	}
	if(i <= page->amount_of_keys && wanted_element == page->info[i]->id) {
		return page->info[i];
	}
	else if(page->leaf) {
		return NULL;
...
void static b_tree_split_child(Page *parent_not_full, uint32_t index_child_full) {
	...
	child_full->amount_of_keys = t - 1;
	for(int j = parent_not_full->amount_of_keys + 1; j < index_child_full + 1; j++) {
		parent_not_full->childs[j+1] = parent_not_full->childs[j];
	}

	parent_not_full->childs[index_child_full+1] = page_new;

	for(int j = parent_not_full->amount_of_keys; j < index_child_full + 1; j++) {
		parent_not_full->childs[j+1] = parent_not_full->childs[j];
	}

	parent_not_full->info[index_child_full] = child_full->info[parent_not_full->amount_of_keys];
	parent_not_full->amount_of_keys ++;
...
void static b_tree_insert_nonfull(Page *page, Row* k) {
	...
	if(page->leaf) {
		...
		if(child->amount_of_keys == MAX_ELEMENTS) {
...
int insert(Page *root, Row *k) {
	if(root->amount_of_keys == MAX_ELEMENTS) {
